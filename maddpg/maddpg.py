...
class MADDPG:
    def __init__(self, env, lr_a=0.01, lr_c=0.01, fc1_dim=64, fc2_dim=64, gamma=0.99, tau=0.01, chkpt_dir='tmp/maddpg/', buffer_size=1000000, batch_size=1024):
        self.env = env
        self.agent_names = env.agents
        self.n_agents = env.num_agents
        self.state_dim = np.prod(env.state().shape)
        self.action_dims = [np.prod(env.action_space(a).shape) for a in self.agent_names]
        self.obs_dims = [np.prod(env.observation_space(a).shape) for a in self.agent_names]
        self.action_boundary = [(env.action_space(a).low, env.action_space(a).high) for a in env.agents]
    ...
    def learn(self, memory):
        ...
        for i in range(self.n_agents):
            ...
            next_q_value = self.agents[i].target_critic(state_, joint_mu_action_).flatten()
            y_target = reward + (1 - done) * self.gamma * next_q_value
            y = self.agents[i].critic(state, joint_action)
    ...
    def train(self, n_episodes, train_freq=100, fig_name='img/simple_adversary.svg', seed=42):
        env = self.env
        ...
        for episode in range(n_episodes):
            ...
            while True:
                ...
                self.replay_buffer.store_transition(state, list(obs.values()), action, list(reward.values()), new_state, list(obs_.values()), done)
            ...
            if (episode + 1) % 50 == 0:
                for a in self.agent_names:
                    print(f'\033[92m\t{sum(reward_data[a][-50:]) / 50:.2f}\033[0m', end='')
