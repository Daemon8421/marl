...
class CriticNetwork(nn.Module):
    def __init__(self, lr, state_dim, action_dims, fc1_dim, fc2_dim, name, chkpt_dir):
        super().__init__()
...
class ActorNetwork(nn.Module):
    def __init__(self, lr, obs_dim, action_dim, fc1_dim, fc2_dim, action_low, action_high, name, chkpt_dir):
        super().__init__()
        ...
        self.device = th.device('cuda:0' if th.cuda.is_available() else 'cpu')
        self.action_low = th.tensor(action_low, dtype=th.float, device=self.device)
        self.action_high = th.tensor(action_high, dtype=th.float, device=self.device)
    ...
    def forward(self, obs):
        ...
        action = self.action_low + ((self.action_high - self.action_low) / 2) * (coarse_action + 1)
