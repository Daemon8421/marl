...
class Agent:
    ...
    def update_parameters(self):
        self.polyak_update(self.actor.parameters(), self.target_actor.parameters())
        self.polyak_update(self.critic.parameters(), self.target_critic.parameters())

    def polyak_update(self, params, target_params):
        with th.no_grad():
            for param, target_param in zip(params, target_params):
                target_param.data.mul_(1 - self.tau)
                th.add(target_param.data, param.data, alpha=self.tau, out=target_param)
